            ...
            <div class="w-11/12 p-12 bg-white sm:w-8/12 md:w-1/2 lg:w-5/12">
                ...
                <form method="POST" action="/users" class="mt-6">
                    ...
                    <span class="w-1/2">
                      ...
                      <input id="name" type="text" name="name" value="{{old('name')}}" placeholder="John Doe" autocomplete="given-name" class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner" required />
                        @error('name')
                            <p class="text-xs italic text-red-500">{{$message}}</p>
                        @enderror
                    </span>
                        <span class="w-1/2">
                          <label for="" class="block text-xs font-semibold text-gray-600 uppercase">Email</label>
                        <input id="email" type="email" name="email" value="{{old('email')}}" placeholder="john.doe@company.com" autocomplete="family-name" class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner" required />
                        @error('email')
                            <p class="text-xs italic text-red-500">{{$message}}</p>
                        @enderror
                        </span>
